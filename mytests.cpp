...
bool testgetEdges() 
{
    ...
    auto edges5 = g.getEdges(5);
    vector<pair<int, double>> expected5 = {{4, 4.8}, {6, 5.5}};
    if (edges5 != expected5) return false;

    auto edges6 = g.getEdges(6);
    if (!edges6.empty()) return false;
...
bool testBFS()
{
    ...
    if (bfsResult[2].second != 1) return false;
    if (bfsResult[6].second != 3) return false;
...
bool testDijkstra()
{
    ...
    if (!(result[2].second > 4.49 && result[2].second < 4.51)) return false;  // Expected ~4.5
    if (!(result[3].second > 1.99 && result[3].second < 2.01)) return false;  // Expected ~2.0
    if (!(result[4].second > 9.89 && result[4].second < 9.91)) return false;  // Expected ~9.9
    if (!(result[5].second > 5.09 && result[5].second < 5.11)) return false;  // Expected ~5.1
    if (!(result[6].second > 10.59 && result[6].second < 10.61)) return false; // Expected ~10.6
